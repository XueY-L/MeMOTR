...
def train_gam(config: dict):
    ...
    grad_rho_scheduler = ProportionScheduler(pytorch_lr_scheduler=scheduler, max_lr=config["LR"], min_lr=config["LR"] * config["LR_DROP_RATE"], max_value=config['grad_rho'], min_value=config['grad_rho'])

    grad_norm_rho_scheduler = ProportionScheduler(pytorch_lr_scheduler=scheduler, max_lr=config["LR"], min_lr=config["LR"] * config["LR_DROP_RATE"], max_value=config['grad_norm_rho'], min_value=config['grad_norm_rho'])

    # GAM
    gam_optimizer = GAM(params=param_groups, base_optimizer=optimizer, model=model, 
                    grad_rho_scheduler=grad_rho_scheduler, grad_norm_rho_scheduler=grad_norm_rho_scheduler, 
                    adaptive=config['adaptive'], args=config, cuda=True)
...
def train_one_epoch(train_states: dict, max_norm: float,
                    dataloader: DataLoader, criterion: ClipCriterion, gam_optimizer: torch.optim,
    ...
    Returns:
        Logs
    """
    gam_optimizer.model.train()
    gam_optimizer.zero_grad()
    device = next(get_model(gam_optimizer.model).parameters()).device
    ...
    for i, batch in enumerate(dataloader):
        ...
        tracks = TrackInstances.init_tracks(batch=batch,
                                            hidden_dim=get_model(gam_optimizer.model).hidden_dim,
                                            num_classes=get_model(gam_optimizer.model).num_classes,
                                            device=device, use_dab=use_dab)
        
        gam_optimizer.set_closure()
        log_dict = gam_optimizer.step(criterion, tracks, batch, device, no_grad_frames, metric_log, accumulation_steps, get_model, tensor_list_to_nested_tensor, i, max_norm)
