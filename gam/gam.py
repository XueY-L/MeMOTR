...
class GAM(torch.optim.Optimizer):
    ...
    def set_closure(self):
        ...
        def get_grad(criterion, tracks, batch, device, no_grad_frames, metric_log, accumulation_steps, get_model, tensor_list_to_nested_tensor):
            self.zero_grad()
                ...
                for frame_idx in range(len(batch["imgs"][0])):
                    if no_grad_frames is None or frame_idx >= no_grad_frames:
                        ...
                        if frame_idx < len(batch["imgs"][0]) - 1:
                            tracks = get_model(self.model).postprocess_single_frame(
    ...
    def step(self, criterion, tracks, batch, device, no_grad_frames, metric_log, accumulation_steps, get_model, tensor_list_to_nested_tensor, i, max_norm, closure=None ):
        if closure:
            get_grad = closure
        else:
        ...
        with self.maybe_no_sync():
            # get gradient
            log_dict = get_grad(criterion, tracks, batch, device, no_grad_frames, metric_log, accumulation_steps, get_model, tensor_list_to_nested_tensor)
            ...
            # model 1
            get_grad(criterion, tracks, batch, device, no_grad_frames, metric_log, accumulation_steps, get_model, tensor_list_to_nested_tensor)
            # grad 1
